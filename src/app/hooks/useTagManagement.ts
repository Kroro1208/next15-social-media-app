...
const tryAddTag = (currentTagIds: number[], newTagId: number) => {
  if (currentTagIds.includes(newTagId)) {
    return null;
  }
  if (currentTagIds.length >= MAX_TAG_SELECTION) {
    toast.error(`タグは最大${MAX_TAG_SELECTION}つまで選択できます`);
    return null;
  }
  return [...currentTagIds, newTagId];
};
...
export const useTagManagement = (
  ...
  useEffect(() => {
    if (!primaryTagId || !watchCommunityId) {
      setRelatedTags([]);
      return;
    }
    ...
    const loadRelatedTags = async () => {
      ...
      try {
        const related = await getRelatedTags(primaryTagId, watchCommunityId, 3);
  ...
  const handleCreateTag = async (
    ...
    try {
      ...
      await refetchTags();
      const updatedSelection = tryAddTag(currentTagIds, newTag.id);
      if (updatedSelection) {
        setTagIds(updatedSelection);
      }
      setNewTagName("");
      setTagValidationError("");
  ...
  const handleSelectSimilarTag = (
    ...
    const updatedSelection = tryAddTag(currentTagIds, tag.id);
    if (updatedSelection) {
      setTagIds(updatedSelection);
    }
    setNewTagName("");
