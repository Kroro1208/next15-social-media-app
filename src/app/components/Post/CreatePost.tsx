...
import type { Community } from "../Community/CommunityList";
import { getCommunitites } from "../Community/CommunityList";
...
const CreatePost = () => {
  ...
  } = useTagManagement(watchCommunityId || null, watchTagIds || []);
  ...
  const handleCreateTagWrapper = async (currentSelectedIds: number[]) => {
    await handleCreateTag(currentSelectedIds, (newValues) =>
      setValue("tag_ids", newValues, { shouldValidate: true }),
    );
  };

  const handleSelectSimilarTagWrapper = (
    tag: { id: number; name: string },
    currentSelectedIds: number[],
  ) => {
    handleSelectSimilarTag(tag, currentSelectedIds, (newValues) =>
      setValue("tag_ids", newValues, { shouldValidate: true }),
    );
  };
